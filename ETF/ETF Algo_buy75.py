...
LATEST-BLOCK BUY PICK (what you asked for)
- Read signals.txt and find the MOST RECENT `file=YYYY-MM-DD` block.
- From that latest block ONLY, shortlist symbols where:
    1) p > 0.75
    2) runupToHigh < 10%   (parsed from `runupToHigh=7.68%`)
- Prompt you to choose from the shortlist (or auto-pick if only 1).
- If you confirm, BUY it and place TP/SL exits (OCO if possible, otherwise fallback).

Replay stays unchanged; this affects only the --ask-to-buy-today prompt behavior.
"""

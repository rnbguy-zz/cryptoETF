...
def btc_market_mood_for_file_date(
    ...
    *,
    ...
    You asked to "send filename date" (day before entry) -> with entry_lag_days=1:
        mood_day = file_day

    General form:
        entry_day = file_day + entry_lag_days
        mood_day  = entry_day - 1 day = file_day + entry_lag_days - 1

    Returns dict:
      { mood: "BULLISH"/"BEARISH"/"NEUTRAL"/"UNKNOWN", o: float|nan, c: float|nan, pct: float|nan, mood_day: "YYYY-MM-DD" }
    """
...
def add_btc_mood_columns_and_filter_dates(
    ...
    *,
    ...
    """
    Adds columns:
      - btc_mood_day (YYYY-MM-DD)
      - btc_open, btc_close, btc_change_pct
      - btc_mood

    Then filters *ENTIRE DATES* based on btc_mood (default you want BULLISH-only).
    mood_filter: "BULLISH" | "BEARISH" | "NEUTRAL" | "ANY"
    """
    if df_pred is None or df_pred.empty:
        return df_pred

    mf = (mood_filter or "BULLISH").strip().upper()
    if mf not in {"BULLISH", "BEARISH", "NEUTRAL", "ANY"}:
        mf = "BULLISH"
    ...
    keep_dates = [d for d in unique_file_dates if results.get(d, {}).get("mood") == mf]
    df_pred = df_pred[df_pred["file_date"].isin(keep_dates)].copy()
    return df_pred
...
def runup_to_high_until_today(
    ...
    *,
    session: requests.Session,
    store_dir: Path,
    include_today: bool = True,   # NEW
) -> Optional[Dict[str, float]]:
    """
    Computes run-up from the ENTRY DAY OPEN to the highest HIGH observed from entry_day
    through:
      - today UTC (including today's in-progress 1d candle) if include_today=True
      - otherwise through the last fully closed UTC day (yesterday UTC)

    IMPORTANT:
    - Requires that the entry_day candle exists. If entry_day is missing (no data),
      returns None rather than silently using a later day.
    """
    ...
    if "day" not in df.columns or "open" not in df.columns or "high" not in df.columns:
        return None
    ...
    # Require exact entry day candle
    ...
    denom = entry_open if abs(entry_open) > 1e-12 else 1e-12
    runup_pct = ((max_high / denom) - 1.0) * 100.0
...
def build_ml_table(
    ...
    *,
    ...
    Performance change:
      - Instead of submitting 1 Future per (symbol, file_day) label job,
        we submit 1 Future per symbol and compute all its labels in a tight loop.
      - This massively reduces ThreadPool scheduling overhead.

    Returns:
      df_all: rows for training + predict windows
      train_dates: list of file dates used for training labels
      holdout_dates: list of file dates used for predict only (label=None)
    """
    ...
    # ------------------------------------------------------------
    # 1) Build all base rows (features) first
    #    - For training dates: we defer label computation
    #    - For holdout/predict dates: label=None
    # ------------------------------------------------------------
    ...
    for fday in file_dates:
        ...
        for sym in sorted(syms):
            ...
            # Keep only oversold hits up to this file date for feature build / min history check
            ...
            if fday in train_set:
                # label later in per-symbol batch
                train_rows_by_symbol[sym].append(base_row)
            else:
                # predict/holdout window: label is None
                base_row["label"] = None
                rows.append(base_row)

    # ------------------------------------------------------------
    # 2) Compute labels per symbol (MUCH fewer Futures)
    # ------------------------------------------------------------
    def _label_symbol_rows(sym: str, sym_rows: List[Dict]) -> List[Dict]:
        """
        Compute labels for all rows of a single symbol in one thread.
        This avoids creating tens of thousands of tiny futures.
        """
        s = get_thread_session()
        out: List[Dict] = []
        ...
        for r in sym_rows:
            ...
            if y is None:
                # same behavior as before: skip rows we can't label
                continue
    ...
    with ThreadPoolExecutor(max_workers=max(1, int(dl_workers))) as ex:
        ...
        for sym, sym_rows in train_rows_by_symbol.items():
            futs.append(ex.submit(_label_symbol_rows, sym, sym_rows))

        for fut in futs:
            rows.extend(fut.result())

    df = pd.DataFrame(rows)
    df = df.sort_values(["file_date", "symbol"]).reset_index(drop=True)
    return df, train_dates, holdout_dates


def add_prev_signal_features_inplace(
    df_all: pd.DataFrame,
    *,
    entry_lag_days: int,
    horizon_days: int,
    ...
    """
    Adds features describing the *previous oversold signal* for the same symbol,
    evaluated as-of the current row's entry time (no lookahead).

    New columns:
      - prev_signal_active (0/1): previous signal hasn't hit target yet as-of (current_entry_day - 1)
      - prev_signal_age_days: current_file_date - prev_file_date
      - prev_signal_runup_so_far_pct: runup from prev entry open to max high so far (up to current_entry_day - 1)
    """
    df_all = df_all.copy()
    df_all["prev_signal_active"] = 0
    df_all["prev_signal_age_days"] = 9999.0
    df_all["prev_signal_runup_so_far_pct"] = 0.0

    if df_all.empty:
        return df_all

    # group row indices per symbol
    by_sym: Dict[str, List[int]] = defaultdict(list)
    for idx, sym in df_all["symbol"].items():
        by_sym[str(sym)].append(int(idx))

    def _work_symbol(sym: str, idxs: List[int]) -> List[Tuple[int, int, float, float]]:
        """
        Returns list of:
          (row_idx, prev_active, prev_age_days, prev_runup_so_far_pct)
        """
        s = get_thread_session()
        out: List[Tuple[int, int, float, float]] = []

        # sort rows by file_date for this symbol
        idxs_sorted = sorted(
            idxs,
            key=lambda i: datetime.strptime(df_all.at[i, "file_date"], "%Y-%m-%d").date(),
        )

        prev_idx: Optional[int] = None

        for idx in idxs_sorted:
            cur_fday = datetime.strptime(df_all.at[idx, "file_date"], "%Y-%m-%d").date()
            cur_entry_day = cur_fday + timedelta(days=entry_lag_days)
            cutoff_day = cur_entry_day - timedelta(days=1)  # last fully known day before entry

            if prev_idx is None:
                out.append((idx, 0, 9999.0, 0.0))
                prev_idx = idx
                continue

            prev_fday = datetime.strptime(df_all.at[prev_idx, "file_date"], "%Y-%m-%d").date()
            age_days = float((cur_fday - prev_fday).days)

            # only consider "yesterday/recent prior signals" within horizon window (tunable)
            if age_days <= 0 or age_days > float(max(1, horizon_days)):
                out.append((idx, 0, age_days, 0.0))
                prev_idx = idx
                continue

            prev_entry_day = prev_fday + timedelta(days=entry_lag_days)

            # Need klines from prev_entry_day .. cutoff_day
            if cutoff_day < prev_entry_day:
                out.append((idx, 0, age_days, 0.0))
                prev_idx = idx
                continue

            dfw = get_klines_window(sym, prev_entry_day, cutoff_day, session=s, store_dir=store_dir)
            if dfw is None or dfw.empty:
                out.append((idx, 0, age_days, 0.0))
                prev_idx = idx
                continue

            entry_open = float(dfw.iloc[0]["open"])
            max_high = float(pd.to_numeric(dfw["high"], errors="coerce").max())
            target = entry_open * (1.0 + float(target_pct))

            hit = (max_high >= target)
            runup_so_far_pct = ((max_high / max(entry_open, 1e-12)) - 1.0) * 100.0

            prev_active = 0 if hit else 1
            out.append((idx, prev_active, age_days, float(runup_so_far_pct)))
    ...
    with ThreadPoolExecutor(max_workers=max(1, int(dl_workers))) as ex:
        futs = [ex.submit(_work_symbol, sym, idxs) for sym, idxs in by_sym.items()]
        for fut in futs:
            for idx, active, age, runup in fut.result():
                df_all.at[idx, "prev_signal_active"] = int(active)
                df_all.at[idx, "prev_signal_age_days"] = float(age)
                df_all.at[idx, "prev_signal_runup_so_far_pct"] = float(runup)

    return df_all


def add_sideways_features_inplace(
    df_all: pd.DataFrame,
    *,
    entry_lag_days: int,
    sideways_days: int,
    sideways_max_range_pct: float,
    sideways_max_abs_change_pct: float,
    target_pct: float,
    horizon_days: int,
    store_dir: Path,
    dl_workers: int,
    ...
    df_all = add_prev_signal_features_inplace(
     df_all,
     entry_lag_days=entry_lag_days,
     horizon_days=horizon_days,
     target_pct=target_pct,
    ...
    if sideways_days <= 0 or df_all.empty:
        return df_all
    ...
    def _work_one_symbol(sym: str, idxs: List[int]) -> List[Tuple[int, Optional[Dict[str, float]], int]]:
        s = get_thread_session()
        out = []
        for idx in idxs:
            fday = datetime.strptime(df_all.at[idx, "file_date"], "%Y-%m-%d").date()
            entry_day = fday + timedelta(days=entry_lag_days)
            ...
            ok = passes_sideways_filter(
                stats,
                max_range_pct=sideways_max_range_pct,
                max_abs_close_change_pct=sideways_max_abs_change_pct,
            )
            out.append((idx, stats, 1 if ok else 0))
        return out

    with ThreadPoolExecutor(max_workers=max(1, int(dl_workers))) as ex:
        futs = [ex.submit(_work_one_symbol, sym, idxs) for sym, idxs in by_sym.items()]
        for fut in futs:
            for idx, stats, ok in fut.result():
                if stats is not None:
                    df_all.at[idx, "sideways_range_pct"] = stats["sideways_range_pct"]
                    df_all.at[idx, "sideways_max_abs_close_change_pct"] = stats["sideways_max_abs_close_change_pct"]
                df_all.at[idx, "sideways_pass"] = ok
...
def build_downside_labels_inplace(
    ...
    *,
    ...
    """
    Faster version:
      - groups df_train rows by symbol
      - submits 1 Future per symbol (not per row)
      - computes labels in a tight loop per symbol

    Output data is the same as before (just faster).
    """
    ...
    def _label_one_symbol(sym: str, idxs: List[int]) -> List[Tuple[int, int]]:
        """
        Returns list of (row_idx, label_int) for rows we could label.
        """
        s = get_thread_session()
        out: List[Tuple[int, int]] = []

        # loop rows for this symbol in one thread
        for idx in idxs:
    ...
    *,
    ...
    def _fmt_price(x) -> str:
        try:
            if pd.isna(x):
                return "NA"
            return f"{float(x):.8g}"  # compact + readable
        except Exception:
            return "NA"
    ...
    for d_str in unique_dates:
        ...
        for _, r in dd.iterrows():
            # Runup fields
            entry_open = r.get("entry_open", np.nan)
            max_high = r.get("max_high_to_today", np.nan)
            runup = r.get("runup_to_high_pct", np.nan)
            maxd = r.get("max_high_date", "")

            entry_open_s = _fmt_price(entry_open)
            max_high_s = _fmt_price(max_high)
            runup_s = _fmt_pct(runup)

            # Drop feature + drop flag
            feat_pdrop = r.get("feat_prob_drop10", np.nan)
            feat_pdrop_s = _fmt_prob(feat_pdrop)

            # Your df_pred has pred_drop_10_next5d when downside model ran
            dropflag = int(r.get("pred_drop_10_next5d", 0) or 0)
            drop_txt = "YES" if dropflag == 1 else "NO"

            lines.append(
                f"BUY  {str(r['symbol']):12s}  p={_fmt_prob(r.get('prob_up_target_h', np.nan))}  "
                f"hits={int(r.get('hits_30d', 0) or 0)}  "
                f"entryOpen={entry_open_s}  maxHigh={max_high_s}  runupToHigh={runup_s}  maxHighDate={maxd}  "
                f"featDropP={feat_pdrop_s}  drop10={drop_txt}"
            )
...
def build_oof_downside_prob_feature(
    ...
    *,
    ...
    # ------------------------------
    # NEW: FAST MODE (SKIP OOF)
    # ------------------------------
    if skip_oof:
        print("\nSKIPPING OOF: training downside model once and using in-sample probs.")

        clf_down, _m = train_and_report(
            usable, "label_down10", feat_cols_down, seed=seed
        )

        # Use same model to fill feature for ALL training rows
    ...
    # ------------------------------
    # ORIGINAL OOF PATH (unchanged logic)
    # ------------------------------
    skf = StratifiedKFold(
...
def main() -> int:
    ...
    ap.add_argument(
     "--btc-mood",
     default="BULLISH",
     help="Filter OUTPUT dates by BTC mood (uses BTCUSDT candle on filename date / day before entry). "
         "Options: BULLISH, BEARISH, NEUTRAL, ANY. Default: BULLISH",
    )

    ap.add_argument(
        "--skip-oof",
        action="store_true",
        help="Skip OOF for downside model; train downside once and use in-sample probs",
    )
    ...
    # ✅ BTC mood filter — OUTPUT ONLY (does NOT touch training)
    df_pred = add_btc_mood_columns_and_filter_dates(
     df_pred,
     store_dir=store_dir,
     entry_lag_days=args.entry_lag_days,
     dl_workers=args.dl_workers,
     mood_filter=args.btc_mood,   # default "BULLISH"
    )
    ...
    if df_pred.empty:
     ...
     for d_str in date_strs:
        ...
        # -------------------------------
        # NEW: BTC mood banner for the date
        # -------------------------------
        ...
        if args.sideways_only:
            print(
                f"\nfile={d.isoformat()}  entry={entry_day.isoformat()}  "
                f"(top {len(dd)})  threshold={args.threshold}  {btc_banner}  "
                f"sideways={args.sideways_days}d range<={args.sideways_max_range_pct:.2f} "
                f"maxAbsClose<={args.sideways_max_abs_change_pct:.2f}"
            )
        else:

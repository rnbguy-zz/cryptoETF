...
def _oversold_folder_fingerprint(folder: "Path") -> dict:
    """
    Fast-ish fingerprint of oversold folder: filename + size + mtime.
    Cache invalidates if any *_oversold.txt changes.
    """
    folder = Path(folder)
...
def _ml_table_cache_key(
    *,
    ...
    """
    Key changes when inputs that affect df_all/features/labels change.
    threshold is intentionally not included (doesn't change labels).
    """
...
def get_or_build_ml_table_cached(
    ...
    *,
    ...
    """
    Wrapper around build_ml_table(...) that caches the *assembled* ML table.
    """
    ...
    # IMPORTANT: call build_ml_table (NOT get_or_build_ml_table_cached)
    ...
    *,
    ...
    def _label_symbol_rows(sym: str, sym_rows: List[Dict]) -> List[Dict]:
        ...
        start = min(entry_days)
            ...
            max(entry_days) + timedelta(days=horizon_days),
        ...
        for r in sym_rows:
            ...
            endw = min(entry + timedelta(days=horizon_days), last_closed)
...
def get_or_train_helper_model(
    *,
    df_train_with_feat: "pd.DataFrame",         # must include label + all feat_cols_main + feat_prob_drop10 if used
    clf_main: "RandomForestClassifier",
    feat_cols_main: "list[str]",
    threshold_main: float,
    helper_cutoff: float,
    train_end_date: "date",
    seed: int = 42,
    helper_bundle_path: "Path" = Path("models/helper_bundle.joblib"),
) -> "tuple[Optional[RandomForestClassifier], Optional[list[str]]]":
    """
    Loads helper ONLY if main fingerprint matches; otherwise trains + saves.
    """
    import json, hashlib
    from pathlib import Path
    import joblib
    ...
    def _rf_params_from_clf(clf) -> dict:
        p = clf.get_params()
        keep = ["n_estimators", "max_depth", "min_samples_leaf", "class_weight", "random_state"]
    ...
    def _rf_helper(seed_: int = 42) -> RandomForestClassifier:
        return RandomForestClassifier(
            n_estimators=400,
            random_state=seed_,
            n_jobs=-1,
    ...
    dfh = df_train_with_feat.dropna(subset=["label"]).copy()
    if dfh.empty:
        print("\nNOTE: Helper skipped: no labeled rows.")
        return None, None

    missing_main = [c for c in feat_cols_main if c not in dfh.columns]
    if missing_main:
        print(f"\nNOTE: Helper skipped: missing main feature cols: {missing_main}")
        return None, None

    main_fp = _hash_obj({
        "train_end_date": train_end_date.isoformat(),
        "threshold_main": float(threshold_main),
        "helper_cutoff": float(helper_cutoff),
        "feat_cols_main": list(feat_cols_main),
        "main_rf_params": _rf_params_from_clf(clf_main),
        "kind": "helper_rf_v1",
    })

    b = _load_bundle(Path(helper_bundle_path))
    if b and b.get("meta", {}).get("main_fingerprint") == main_fp:
        print(f"[CACHE] Helper model hit: {helper_bundle_path}", flush=True)
        return b.get("helper_model"), b.get("helper_feature_cols")

    # --- train helper ---
    X = dfh[feat_cols_main].astype(float).values
    y = dfh["label"].astype(int).values
    p_main = clf_main.predict_proba(X)[:, 1]
    pred_main = (p_main >= float(threshold_main)).astype(int)
    mistake = (pred_main != y).astype(int)

    if len(np.unique(mistake)) < 2:
        print("\nNOTE: Helper skipped: mistake labels have <2 classes.")
        return None, None

    dfh2 = dfh.copy()
    dfh2["p_main"] = p_main
    helper_feature_cols = list(feat_cols_main) + ["p_main"]

    Xh = dfh2[helper_feature_cols].astype(float).values
    helper = _rf_helper(seed)
    helper.fit(Xh, mistake)

    _save_bundle(Path(helper_bundle_path), {
        "helper_model": helper,
        "helper_feature_cols": helper_feature_cols,
        "meta": {
    ...
    print(f"[CACHE] Helper model saved: {helper_bundle_path}", flush=True)
    return helper, helper_feature_cols


def get_or_train_downside_model_and_feature(
    *,
    df_train_labeled: "pd.DataFrame",     # must include base features + label_down10 (or build it before calling)
    ...
    train_end_date: "date",
    seed: int = 42,
    downside_bundle_path: "Path" = Path("models/downside_bundle.joblib"),
) -> "tuple[pd.DataFrame, Optional[RandomForestClassifier], Optional[list[str]]]":
    """
    Loads or trains the downside model (fixed defaults), then fills df['feat_prob_drop10'] for training rows
    using in-sample probabilities (skip-oof behaviour).

    Returns:
      df_train_with_feat, clf_down, feat_cols_down
    """
    import json, hashlib
    from pathlib import Path
    import joblib
    ...
    def _rf_down(seed_: int = 42) -> RandomForestClassifier:
        return RandomForestClassifier(
            n_estimators=500,
            random_state=seed_,
            n_jobs=-1,
    ...
    # --- ensure label_down10 exists ---
    df_train_labeled = df_train_labeled.copy()
    if "label_down10" not in df_train_labeled.columns or df_train_labeled["label_down10"].isna().all():
    ...
    want_meta = {
        ...
        "seed": int(seed),
        "model_kind": "downside_rf_v1",
    }

    clf_down = None
    feat_cols_down = None

    b = _load_bundle(Path(downside_bundle_path))
    if b:
        if b.get("meta") == want_meta:
            clf_down = b.get("down_model")
            feat_cols_down = b.get("down_feature_cols")

    # --- train if missing ---
    df_out = df_train_labeled.copy()
    df_out["feat_prob_drop10"] = np.nan

    usable = df_out.dropna(subset=["label_down10"]).copy()
    if clf_down is None or feat_cols_down is None:
        ...
        clf_down = _rf_down(seed)
        Xd = usable[feat_cols_down].astype(float).values
        yd = usable["label_down10"].astype(int).values
        clf_down.fit(Xd, yd)

        _save_bundle(Path(downside_bundle_path), {
            "down_model": clf_down,
            "down_feature_cols": feat_cols_down,
            "meta": want_meta,
        })
        print(f"[CACHE] Downside model saved: {downside_bundle_path}", flush=True)
    else:
        print(f"[CACHE] Downside model hit: {downside_bundle_path}", flush=True)

    # --- fill in-sample probs on rows with label_down10 ---
    if not usable.empty and clf_down is not None and feat_cols_down is not None:
        Xd = usable[feat_cols_down].astype(float).values
        probs = clf_down.predict_proba(Xd)[:, 1]
        df_out.loc[usable.index, "feat_prob_drop10"] = probs

    return df_out, clf_down, feat_cols_down



def apply_helper_to_df(
    df: pd.DataFrame,
    *,
    clf_main: RandomForestClassifier,
    feat_cols_main: List[str],
    helper_clf: Optional[RandomForestClassifier],
    helper_feature_cols: Optional[List[str]],
    helper_cutoff: float,
) -> pd.DataFrame:
    """
    Adds:
      - prob_up_target_h (main prob) if missing
      - p_mistake (helper prob) if helper exists
    Returns df with new columns; does NOT filter unless you filter after.
    """
    if df is None or df.empty:
        return df
    ...
    # Ensure main prob exists
    if "prob_up_target_h" not in out.columns:
        X = out[feat_cols_main].astype(float).values
        out["prob_up_target_h"] = clf_main.predict_proba(X)[:, 1]

    # Helper score
    out["p_mistake"] = np.nan
    out["helper_cutoff"] = helper_cutoff

    if helper_clf is not None and helper_feature_cols is not None:
        missing = [c for c in helper_feature_cols if c not in out.columns]
        if missing:
            # If p_main is expected in helper_feature_cols, it may not exist yet:
            # We'll create it from prob_up_target_h if needed.
            if "p_main" in missing:
                out["p_main"] = out["prob_up_target_h"].astype(float)
                missing = [c for c in helper_feature_cols if c not in out.columns]

        if not missing:
            Xh = out[helper_feature_cols].astype(float).values
            out["p_mistake"] = helper_clf.predict_proba(Xh)[:, 1]

    return out


def final_system_report_on_validation(
    *,
    df_va: pd.DataFrame,
    y_va: np.ndarray,
    p_main_va: np.ndarray,
    helper_clf: Optional[RandomForestClassifier],
    helper_feature_cols: Optional[List[str]],
    helper_cutoff: float,
    threshold_main: float,
    ...
    """
    Prints ONE report for the FINAL system:

      main_pred = (p_main >= threshold_main)
      final_pred = main_pred AND (p_mistake < helper_cutoff)

    Uses validation split only (labels are known).
    """
    from sklearn.metrics import classification_report

    # main predictions
    main_pred = (p_main_va >= threshold_main).astype(int)

    # default: no helper => final == main
    final_pred = main_pred.copy()

    if helper_clf is not None and helper_feature_cols is not None and len(df_va) > 0:
        # Build a temp frame for helper features
        tmp = df_va.copy()
        tmp["p_main"] = p_main_va  # ALWAYS provide p_main to helper

        missing = [c for c in helper_feature_cols if c not in tmp.columns]
        if not missing:
            Xh = tmp[helper_feature_cols].astype(float).values
            p_mistake = helper_clf.predict_proba(Xh)[:, 1]
            keep = (p_mistake < helper_cutoff).astype(int)
            final_pred = (main_pred & keep).astype(int)

    print("\n=== FINAL SYSTEM (Main + Helper filter) Validation ===")
    print(f"main_threshold={threshold_main}  helper_cutoff={helper_cutoff}")
    print(classification_report(y_va, final_pred, digits=digits))

    # Also print “BUY precision” explicitly (precision of class 1)
    # precision_1 = TP / (TP + FP)
    tp = int(((final_pred == 1) & (y_va == 1)).sum())
    fp = int(((final_pred == 1) & (y_va == 0)).sum())
    buy_precision = tp / max(1, (tp + fp))
    print(f"FINAL BUY precision (class 1 precision): {buy_precision:.{digits}f}")
...
def train_main_model_with_drop_feature(
    ...
    *,
    threshold_main: float,
    helper_cutoff: float = 0.65,
    seed: int = 42,
) -> Tuple[RandomForestClassifier, List[str], Dict, Optional[RandomForestClassifier], Optional[List[str]]]:
    """
    Trains:
      - main model (label)
      - helper model to predict "mistake risk" of the main model, using training split only

    Prints ONLY:
      - main model validation report (existing)
      - ONE final combined system report (main + helper filter)

    Returns:
      clf_main, feat_cols_main, main_metrics, helper_clf, helper_feature_cols
    """
    ...
    feat_cols_main = _get_feature_cols(df_train, ("label", "label_down10"))
    if "feat_prob_drop10" not in feat_cols_main and "feat_prob_drop10" in df_train.columns:
        feat_cols_main.append("feat_prob_drop10")

    # ---- single split used for main+helper evaluation ----
    y = df_train["label"].astype(int).values
    X = df_train[feat_cols_main].astype(float).values

    X_tr, X_va, y_tr, y_va, df_tr, df_va = train_test_split(
        X, y, df_train, test_size=0.2, random_state=seed, stratify=y
    )

    # ---- train main on train split ----
    clf_main = _rf_default(seed=seed)
    clf_main.fit(X_tr, y_tr)

    p_va = clf_main.predict_proba(X_va)[:, 1]
    y_hat = (p_va >= 0.5).astype(int)
    ...
    # Keep your existing print style for main model validation:
    print(f"\n=== Validation (random split sanity) [label] ===")
    print(f"AUC: {auc:.4f}")
    print(classification_report(y_va, y_hat, digits=4))
    ...
    # ---- train helper on TRAIN split only (quietly) ----
    # Helper target: mistake = 1 if main prediction wrong on TRAIN split rows
    p_tr = clf_main.predict_proba(X_tr)[:, 1]
    pred_tr = (p_tr >= threshold_main).astype(int)  # IMPORTANT: use your operational threshold
    mistake_tr = (pred_tr != y_tr).astype(int)

    # Helper features = main features + p_main
    df_tr2 = df_tr.copy()
    df_tr2["p_main"] = p_tr

    helper_feature_cols = feat_cols_main + ["p_main"]

    # Can we train helper?
    helper_clf: Optional[RandomForestClassifier] = None
    if len(np.unique(mistake_tr)) >= 2:
        Xh_tr = df_tr2[helper_feature_cols].astype(float).values
        helper_clf = RandomForestClassifier(
            n_estimators=400,
            random_state=seed,
            n_jobs=-1,
            class_weight="balanced_subsample",
        ...
        helper_clf.fit(Xh_tr, mistake_tr)

    # ---- FINAL combined system report (ONE report) ----
    final_system_report_on_validation(
        df_va=df_va,
        y_va=y_va,
        p_main_va=p_va,
        helper_clf=helper_clf,
        helper_feature_cols=helper_feature_cols if helper_clf is not None else None,
        helper_cutoff=helper_cutoff,
        threshold_main=threshold_main,
        digits=4,
    )

    # ---- retrain main on full training set for production ----
    clf_main_full = _rf_default(seed=seed)
    clf_main_full.fit(X, y)

    # ---- retrain helper on full training set for production (if possible) ----
    helper_full: Optional[RandomForestClassifier] = None
    if helper_clf is not None:
        p_full = clf_main_full.predict_proba(X)[:, 1]
        pred_full = (p_full >= threshold_main).astype(int)
        mistake_full = (pred_full != y).astype(int)

        if len(np.unique(mistake_full)) >= 2:
            df_full2 = df_train.copy()
            df_full2["p_main"] = p_full
            Xh_full = df_full2[helper_feature_cols].astype(float).values

            helper_full = RandomForestClassifier(
                n_estimators=400,
                random_state=seed,
                n_jobs=-1,
                class_weight="balanced_subsample",
            ...
            helper_full.fit(Xh_full, mistake_full)

    return clf_main_full, feat_cols_main, main_metrics, helper_full, (helper_feature_cols if helper_full is not None else None)
    ...
    for d_str in sorted(df_pred["file_date"].unique().tolist()):
        ...
        for _, r in dd.iterrows():
            lines.append(
                ...
                f"risk={_fmt_prob(r.get('p_mistake', np.nan))}  "
...
def main() -> int:
    ...
    if not need_retrain and bundle is not None:
        ...
        helper_clf = bundle.get("helper_model")
        helper_feature_cols = bundle.get("helper_feature_cols")
        helper_cutoff = float(bundle.get("helper_cutoff", 0.65))
        print(
        ...
        # Downside labels
        # Downside labels (creates label_down10)
        df_train_labeled = build_downside_labels_inplace(
            df_train,
            store_dir=store_dir,
            drop_pct=DROP_PCT,
        ...
        # Base features (exclude labels)
        base_feature_cols = _get_feature_cols(df_train_labeled, ("label", "label_down10"))

        # Train/load downside model + create feat_prob_drop10 (skip-oof behavior)
        df_train_with_feat, clf_down, feat_cols_down = get_or_train_downside_model_and_feature(
        ...
        # Train main model (and helper model inside this function)
        clf_main, feat_cols_main, main_metrics, helper_clf, helper_feature_cols = train_main_model_with_drop_feature(
            df_train_with_feat,
            threshold_main=args.threshold,
            helper_cutoff=0.65,
            seed=42,
        )
    ...
    # Add helper scores + filter final BUYs
    helper_cutoff = float(bundle.get("helper_cutoff", 0.65)) if bundle else 0.65

    df_pred = apply_helper_to_df(
     df_pred,
     clf_main=clf_main,
     feat_cols_main=feat_cols_main,
     helper_clf=helper_clf,
     helper_feature_cols=helper_feature_cols,
     helper_cutoff=helper_cutoff,
    )

    # FINAL RULE: keep only BUYs the helper does NOT think are mistakes
    if "p_mistake" in df_pred.columns and df_pred["p_mistake"].notna().any():
     df_pred = df_pred[(df_pred["pred_buy"] == 1) & (df_pred["p_mistake"] < helper_cutoff)].copy()
    else:

...
def _normalize_store_df(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty:
        return pd.DataFrame(
            columns=["day", "open_time", "open", "high", "low", "close", "close_time", "volume", "no_data"]
        )
    ...
    for c in ["open_time", "open", "high", "low", "close", "close_time", "volume", "no_data"]:
    ...
    # Make numeric columns numeric (leave NaNs as NaNs)
...
def add_waitperiod_volume_motion_features_inplace(
    df: pd.DataFrame,
    *,
    waitperiod_days: int,
    entry_lag_days: int,
    ...
    """
    Adds your features (using YOUR formulas) onto df rows, anchored on ENTRY day:

      start_day = file_date + entry_lag_days
      end_day   = start_day + waitperiod_days

      Velcoity     = Volume[end] - Volume[start]
      acceleration = (Volume[end] - Volume[start]) / waitperiod_days
      timetostop   = Velcoity / acceleration
      Slope20      = ((Close[end] - Open[start]) / Open[start]) * 100

    If any needed day is missing -> NaN.
    """
    if df is None or df.empty:
        return df
    ...
    def _one_symbol(sym: str, idxs: List[int]) -> List[Tuple[int, float, float, float, float]]:
        s = get_thread_session()
        out: List[Tuple[int, float, float, float, float]] = []

        file_days = [datetime.strptime(df.at[i, "file_date"], "%Y-%m-%d").date() for i in idxs]
        start_days = [d + timedelta(days=entry_lag_days) for d in file_days]

        start_min = min(start_days)
        end_max = min(max(start_days) + timedelta(days=waitperiod_days), last_closed)

        full = ensure_symbol_days(sym, start_min, end_max, session=s, store_dir=store_dir)
        ...
        full = full[full["day"].notna()].copy()
        if full.empty:
            return out

        full = full.sort_values("day").drop_duplicates("day", keep="last").set_index("day", drop=True)

        for i in idxs:
            file_day = datetime.strptime(df.at[i, "file_date"], "%Y-%m-%d").date()
            start_day = file_day + timedelta(days=entry_lag_days)
            end_day = start_day + timedelta(days=waitperiod_days)

            if end_day > last_closed:
                continue

            if start_day not in full.index or end_day not in full.index:
                continue

            try:
                vol0 = float(full.at[start_day, "volume"])
                vol1 = float(full.at[end_day, "volume"])
                open0 = float(full.at[start_day, "open"])
                close1 = float(full.at[end_day, "close"])
            except Exception:
                continue
            ...
            slope20 = ((close1 - open0) / open0) * 100.0 if open0 and np.isfinite(open0) else np.nan

            out.append((i, float(velocity), float(acceleration), float(timetostop), float(slope20)))

        return out

    with ThreadPoolExecutor(max_workers=max(1, int(dl_workers))) as ex:
        futs = [ex.submit(_one_symbol, sym, idxs) for sym, idxs in by_sym.items()]
        for fut in futs:
            for idx, v, a, t, s20 in fut.result():
                df.at[idx, "Velcoity"] = v
                df.at[idx, "acceleration"] = a
                df.at[idx, "timetostop"] = t
                df.at[idx, "Slope20"] = s20
...
def get_klines_window(
    ...
    *,
    ...
    out = full.loc[m, ["day", "open", "high", "low", "close", "volume", "open_time", "close_time"]].copy()  # ✅ volume
    if out.empty:
        return out
...
def build_ml_table(
    ...
    *,
    ...
    # Build base rows (features) for all file dates; only training dates get labeled later
    ...
    # ✅ ADD your waitperiod=20 features here
    df = add_waitperiod_volume_motion_features_inplace(
        df,
        waitperiod_days=20,
...
def _gate_candidate_and_maybe_replace_default(
    *,
    ...
    - If candidate passes gate -> save as DEFAULT model and use it
    - If candidate fails gate -> load DEFAULT model if available, else fall back to candidate
    """
...
def main() -> int:
    ...
    try:
        ...
        prefetch_symbol_windows(
            hits,
            entry_lag_days=ENTRY_LAG_DAYS,
            horizon_days=max(HORIZON_DAYS, DROP_HORIZON_DAYS, 20),
            store_dir=store_dir,
    ...
    print("non-null Velocity:", df_all["Velcoity"].notna().sum())
    print("non-null accel:", df_all["acceleration"].notna().sum())
    print("non-null timetostop:", df_all["timetostop"].notna().sum())
    print("non-null Slope20:", df_all["Slope20"].notna().sum())
    ...
    if not need_retrain and bundle is not None:
        ...
        print(df_train_with_feat[["Velcoity","acceleration","timetostop","Slope20"]].describe(include="all"))
        ...
        print([c for c in ["Velcoity","acceleration","timetostop","Slope20"] if c in feat_cols_main])

...
def _normalize_store_df(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty:
        return pd.DataFrame(
            columns=["day", "open_time", "open", "high", "low", "close", "volume", "close_time", "no_data"]
        )
    ...
    required = ["open_time", "open", "high", "low", "close", "volume", "close_time", "no_data"]
    for c in required:
...
def get_klines_window(
    ...
    *,
    ...
    """
    Returns only days with actual OHLCV data (no tombstones), inclusive start_day..end_day.
    """
    if end_day < start_day:
        return pd.DataFrame()
    ...
    out = full.loc[m, ["day", "open", "high", "low", "close", "volume", "open_time", "close_time"]].copy()
    if out.empty:
        return out
    out["day"] = out["day"].astype(str)
    return out.sort_values("day").reset_index(drop=True)

def price_context_features_for_row(
    symbol: str,
    file_day: date,
    *,
    session: requests.Session,
    ...
    """
    Computes features at ENTRY_DAY = file_day + entry_lag_days:

      trend_7d  = close(entry) / close(entry-7)  - 1
      trend_30d = close(entry) / close(entry-30) - 1

      range_7d = (max(high over [entry-7..entry]) / min(low over [entry-7..entry])) - 1

      vol_ratio_3d = avg(volume over [entry-2..entry]) / avg(volume over [entry-29..entry])

      rel_str_7d = (alt_return_7d - btc_return_7d)
        where alt_return_7d = close(entry)/close(entry-7) - 1
              btc_return_7d = close(entry)/close(entry-7) - 1

    Returns {} if not enough data.
    """
    entry_day = file_day + timedelta(days=int(entry_lag_days))
    start_day = entry_day - timedelta(days=30)
    end_day = entry_day

    def _prep_df(sym: str) -> pd.DataFrame:
        df = get_klines_window(sym, start_day, end_day, session=session, store_dir=store_dir)
        if df is None or df.empty:
            return pd.DataFrame()
        df = df.copy()
        ...
        if "volume" in df.columns:
            ...
            df["volume"] = np.nan
        df = df.dropna(subset=["day", "close", "high", "low"])
        if df.empty:
            return pd.DataFrame()
    ...
    btc = _prep_df("BTCUSDT")  # needed for rel strength
    # btc can be empty (API hiccup) — we’ll just skip rel_str then

    # entry close
    row_e = alt[alt["day"] == entry_day]
    if row_e.empty:
        return {}
    close_e = float(row_e.iloc[-1]["close"])
    if close_e <= 0:
        return {}

    def _close_on(df: pd.DataFrame, day: date) -> Optional[float]:
        r = df[df["day"] == day]
        if r.empty:
            return None
        v = r.iloc[-1]["close"]
    ...
    # trend
    c7 = _close_on(alt, entry_day - timedelta(days=7))
    c30 = _close_on(alt, entry_day - timedelta(days=30))
    if c7 is not None:
        feats["trend_7d"] = float((close_e / c7) - 1.0)
    if c30 is not None:
        feats["trend_30d"] = float((close_e / c30) - 1.0)

    # range_7d
    w7 = alt[(alt["day"] >= (entry_day - timedelta(days=7))) & (alt["day"] <= entry_day)]
    if not w7.empty:
        max_high = float(w7["high"].max())
        min_low = float(w7["low"].min())
        if min_low > 0:
            feats["range_7d"] = float((max_high / min_low) - 1.0)

    # volume ratio
    if "volume" in alt.columns:
        w3 = alt[(alt["day"] >= (entry_day - timedelta(days=2))) & (alt["day"] <= entry_day)]
        w30v = alt[(alt["day"] >= (entry_day - timedelta(days=29))) & (alt["day"] <= entry_day)]
        if not w3.empty and not w30v.empty:
            v3 = pd.to_numeric(w3["volume"], errors="coerce").dropna()
            v30 = pd.to_numeric(w30v["volume"], errors="coerce").dropna()
            if len(v3) >= 2 and len(v30) >= 10:  # avoid garbage ratios
                av3 = float(v3.mean())
                av30 = float(v30.mean())
                if av30 > 0:
                    feats["vol_ratio_3d"] = float(av3 / av30)

    # relative strength vs BTC (7d)
    if not btc.empty and c7 is not None:
        btc_e = _close_on(btc, entry_day)
        btc_7 = _close_on(btc, entry_day - timedelta(days=7))
        if btc_e is not None and btc_7 is not None:
            alt_ret7 = (close_e / c7) - 1.0
            btc_ret7 = (btc_e / btc_7) - 1.0
            feats["rel_str_7d"] = float(alt_ret7 - btc_ret7)
...
def build_features_for_symbol(history: List[OversoldHit], asof_file_date: date) -> Dict[str, float]:
    ...
    feats = {
        ...
        # NEW: hit density pressure
        "hits_density_30d": float(hits_30d / 30.0),
        "hits_density_60d": float(hits_60d / 60.0),
    }
    ...
    *,
    ...
    for fday in file_dates:
        ...
        for sym in sorted(syms):
            ...
            # NEW: price/trend context features (uses cached klines store)
            try:
                s = get_thread_session()
                px = price_context_features_for_row(
                 sym,
                 fday,
                 session=s,
                 store_dir=store_dir,
    ...
    def _label_symbol_rows(sym: str, sym_rows: List[Dict]) -> List[Dict]:
        ...
        # compute the whole date range needed for this symbol
...
def train_and_report(
    ...
    debug_top_n: int = 300,
) -> Tuple[RandomForestClassifier, Dict, Optional[pd.DataFrame]]:
    """
    Returns (trained_model, metrics_dict, debug_df_or_None)

    debug_df contains validation rows with:
      - y_true, p, y_pred
      - error_type in {TP, TN, FP, FN}
      - plus file_date/symbol if present
    ...
    debug_df = None
    if return_debug:
        df_va = df_train.loc[idx_va].copy()
        df_va["y_true"] = y_va
        df_va["p"] = p_va
        df_va["y_pred"] = y_hat
        ...
        df_va["error_type"] = [
            _etype(int(t), int(p)) for t, p in zip(df_va["y_true"].values, df_va["y_pred"].values)
        ]

        # make the most “interesting” mistakes float to top
        # FP: highest p first, FN: lowest p first
        fp = df_va[df_va["error_type"] == "FP"].sort_values("p", ascending=False)
        fn = df_va[df_va["error_type"] == "FN"].sort_values("p", ascending=True)

        # keep some TN/TP too if you want to compare distributions
        debug_df = pd.concat([fp.head(debug_top_n), fn.head(debug_top_n)], ignore_index=True)

    return clf, metrics, debug_df
...
def build_downside_prob_feature_skip_oof(
    ...
    *,
    ...
    clf_down, _m, _ = train_and_report(usable, "label_down10", feat_cols_down, seed=seed)
...
def train_main_model_with_drop_feature(
    ...
    *,
    ...
    # NEW: derived features from downside model output (safe + interaction)
    df_train["feat_safe"] = 1.0 - df_train["feat_prob_drop10"].astype(float)
    df_train["feat_safe_x_drop"] = df_train["feat_safe"].astype(float) * df_train["last_drop"].astype(float)

    feature_cols = _get_feature_cols(df_train, ("label", "label_down10"))

    # Ensure downside + derived features are included
    for c in ["feat_prob_drop10", "feat_safe", "feat_safe_x_drop"]:
        if c not in feature_cols and c in df_train.columns:
            feature_cols.append(c)

    clf, metrics, _ = train_and_report(df_train, "label", feature_cols, seed=seed)
...
def prefetch_symbol_windows(
    ...
    *,
    ...
    """
    Prefetches required kline windows for every symbol in hits for:
      entry_day = file_date + entry_lag_days
      window = [entry_day-30 .. entry_day+horizon_days]   (30 for context features)
    Also prefetches BTCUSDT over the union range (needed for rel_str_7d / btc mood).
    Returns dict symbol -> (start_day, end_day) used.
    """
    by_sym: Dict[str, Tuple[date, date]] = {}

    for h in hits:
        entry_day = h.file_date + timedelta(days=int(entry_lag_days))
        start_day = entry_day - timedelta(days=30)
        end_day = entry_day + timedelta(days=int(horizon_days))

        cur = by_sym.get(h.symbol)
        if cur is None:
            by_sym[h.symbol] = (start_day, end_day)
        else:
            by_sym[h.symbol] = (min(cur[0], start_day), max(cur[1], end_day))

    # Add BTCUSDT over the union range (for rel strength + btc mood)
    if by_sym:
        d0 = min(r[0] for r in by_sym.values())
        d1 = max(r[1] for r in by_sym.values())
        by_sym["BTCUSDT"] = (d0, d1)

    def _task(sym: str, rng: Tuple[date, date]) -> None:
        s = get_thread_session()
        ensure_symbol_days(sym, rng[0], rng[1], session=s, store_dir=store_dir)

    with ThreadPoolExecutor(max_workers=max(1, int(dl_workers))) as ex:
        futs = [ex.submit(_task, sym, rng) for sym, rng in by_sym.items()]
        for fut in futs:
            fut.result()
...
def predict_dates(
    ...
    *,
    ...
    Adds:
      - prob_up_target_h
      - pred_buy (1 if prob >= threshold)
    Returns only rows from predict_dates_list.
    """
    if df_all is None or df_all.empty or not predict_dates_list:
        return pd.DataFrame()

    want = set(d.isoformat() for d in predict_dates_list)
    dfp = df_all[df_all["file_date"].isin(want)].copy()
    if dfp.empty:
        return dfp

    missing = [c for c in feature_cols if c not in dfp.columns]
    if missing:
        raise RuntimeError(f"Prediction failed: missing feature columns: {missing}")

    X = dfp[feature_cols].astype(float).values
    p = clf.predict_proba(X)[:, 1]
    dfp["prob_up_target_h"] = p
    dfp["pred_buy"] = (dfp["prob_up_target_h"] >= float(threshold)).astype(int)
    return dfp
...
def main() -> int:
    ...
    try:
     ...
     need_ranges = prefetch_symbol_windows(
        hits,
        entry_lag_days=ENTRY_LAG_DAYS,
    ...
    # NEW: derived features must exist for prediction too
    df_all["feat_safe"] = 1.0 - pd.to_numeric(df_all["feat_prob_drop10"], errors="coerce")
    df_all["feat_safe_x_drop"] = df_all["feat_safe"].astype(float) * pd.to_numeric(df_all["last_drop"], errors="coerce")

...
- The bundle is a dict and contains:
    - main_model, main_feature_cols
    - down_model, down_feature_cols
    - meta (optional)

Usage:
  python rank_oversold_predict_only.py --dir oversold_analysis --threshold 0.61
...
def build_predict_feature_table(
    ...
    *,
    ...
    return pd.DataFrame(rows).sort_values(["file_date", "symbol"]).reset_index(drop=True)
...
# ============================================================
# ============================================================
# MODEL LOAD + PREDICT (MAIN MODEL ONLY)
# ============================================================
...
def load_model_bundle(path: Path) -> Dict:
    if not Path(path).exists():
        raise RuntimeError(f"No model bundle found at: {path}")
    try:
        obj = joblib.load(path)
    except Exception as e:
        raise RuntimeError(f"[MODEL] Failed to load {path}: {e}") from e
    if not isinstance(obj, dict):
        raise RuntimeError(f"[MODEL] Bundle at {path} is not a dict. Got: {type(obj)}")
    return obj


def _pick_first_key(d: dict, keys: list[str]):
    for k in keys:
        if k in d:
            return d[k]
    return None
...
def load_main_bundle(*, bundle_path: Path) -> tuple[object, list[str], dict]:
    """
    Loads bundle and returns:
      (main_model, main_feature_cols, meta)

    Accepts common key aliases:
      model: main_model | clf_main | model
      feature cols: main_feature_cols | feat_cols_main | feature_cols
      meta: meta (optional)
    """
    bundle = load_model_bundle(bundle_path)

    main_model = _pick_first_key(bundle, ["main_model", "clf_main", "model"])
    main_feature_cols = _pick_first_key(bundle, ["main_feature_cols", "feat_cols_main", "feature_cols"])
    meta = bundle.get("meta", {})
    if not isinstance(meta, dict):
        meta = {}

    if main_model is None:
        raise RuntimeError(f"Bundle missing main model key. Found keys={list(bundle.keys())}")
    if not hasattr(main_model, "predict_proba"):
        raise RuntimeError(f"Main model has no predict_proba(): {type(main_model)}")

    if not isinstance(main_feature_cols, list) or not main_feature_cols or not all(isinstance(x, str) for x in main_feature_cols):
        raise RuntimeError("Main feature cols missing or invalid. Expected non-empty list[str].")

    return main_model, list(main_feature_cols), meta
...
def predict_on_pred_feats(
    *,
    ...
    """
    Predict-only using MAIN model only.
    Raises if any required feature columns are missing.
    """
...
def build_email_body(
    *,
    ...
    if predict_dates_list:
        lines.append(f"Predict dates: {predict_dates_list[0]} .. {predict_dates_list[-1]}  ({len(predict_dates_list)} days)")
    else:
...
def main() -> int:
    ...
    ap.add_argument("--model-bundle", default=str(DEFAULT_MODEL_BUNDLE_PATH), help="Path to saved model bundle joblib")
    ...
    print(f"Predict dates: {predict_dates_list[0]} .. {predict_dates_list[-1]}  ({len(predict_dates_list)} days)")

    # 3) Load model bundle (your retrained default_model_76.joblib by default)
    # 3) Load model (main only)
    main_model, main_feature_cols, model_meta = load_main_bundle(bundle_path=DEFAULT_MODEL_BUNDLE_PATH)

    # 4) Predict on those rows (main only)

...
##NEW
# ============================================================
# INCOMPLETE-DATA GUARDS (ADD THIS NEAR YOUR HELPERS)
# ============================================================

# Strictness:
# - Sweep/training: keep 0 (skip any row with missing days)
# - Live scoring (if you later reuse label logic): you could relax to 1
MAX_MISSING_DAYS_UP = 0
MAX_MISSING_DAYS_DOWN = 0

# If a window is shorter than this fraction of expected days, treat as incomplete no matter what
MIN_COVERAGE_FRAC = 1.0  # 1.0 = must be complete

def _coerce_window_days(df_window: pd.DataFrame) -> pd.DataFrame:
    """Ensure df_window has a clean 'day_dt' (date objects) column for checks."""
    ww = df_window.copy()
    if "day" not in ww.columns:
        ww["day_dt"] = pd.NaT
        return ww
    ww["day_dt"] = pd.to_datetime(ww["day"], errors="coerce").dt.date
    ww = ww[ww["day_dt"].notna()].copy()
    return ww
...
def _window_is_complete(
    ...
    *,
    ...
    """
    Returns: (ok, missing_days_list, have_unique_days, expected_days)
    """
    if end_day < start_day:
        return False, [], 0, 0
    if df_window is None or df_window.empty:
        expected = (end_day - start_day).days + 1
        return False, [start_day.isoformat()], 0, expected

    ww = _coerce_window_days(df_window)
    if ww.empty:
        expected = (end_day - start_day).days + 1
        return False, [start_day.isoformat()], 0, expected

    # Use your existing helper (expects df with 'day' strings). We'll feed it "day_dt" too.
    # We'll create a tiny view with 'day' as strings for compatibility.
    tmp = ww[["day_dt"]].rename(columns={"day_dt": "day"}).copy()
    tmp["day"] = tmp["day"].astype(str)

    missing = _missing_days_in_window(tmp, start_day, end_day)
    expected_days = (end_day - start_day).days + 1
    have_unique = int(tmp["day"].nunique())

    # Coverage check (belt + suspenders)
    coverage = float(have_unique) / float(expected_days) if expected_days else 0.0
    if coverage < float(min_coverage_frac):
        return False, missing, have_unique, expected_days

    if len(missing) > int(max_missing_days):
        return False, missing, have_unique, expected_days

    # Also require that start_day exists (prevents "window shifted forward" problems)
    have_days = set(pd.to_datetime(tmp["day"], errors="coerce").dt.date.dropna().tolist())
    if start_day not in have_days:
        return False, missing, have_unique, expected_days

    return True, missing, have_unique, expected_days

def _prices_are_usable(df_window: pd.DataFrame) -> bool:
    """Reject windows with non-finite entry open or totally broken highs/lows."""
    if df_window is None or df_window.empty:
        return False
    try:
        entry_open = float(df_window.iloc[0]["open"])
        if (not np.isfinite(entry_open)) or abs(entry_open) < 1e-12:
            return False

        highs = pd.to_numeric(df_window.get("high"), errors="coerce")
        lows = pd.to_numeric(df_window.get("low"), errors="coerce")

        if highs is None or lows is None:
            return False

        # Need at least one finite high and one finite low in the window
...
def label_drop_pct_within_horizon(
    ...
    *,
    ...
    start_day = entry_day
    end_day_raw = entry_day + timedelta(days=int(horizon_days))
    end_day = min(end_day_raw, last_closed)
    if end_day < start_day:
        return None
    ...
    # ---- NEW: completeness + sanity checks ----
    ok, missing, have_u, exp = _window_is_complete(
        df, start_day, end_day,
        max_missing_days=MAX_MISSING_DAYS_DOWN,
        min_coverage_frac=MIN_COVERAGE_FRAC,
    )
    if not ok or not _prices_are_usable(df):
        # Optional: uncomment for debugging bad periods
        # print(f"[SKIP DOWN] {symbol} file={file_day} entry={start_day} end={end_day} missing={len(missing)} have={have_u}/{exp}")
        return None
...
DEFAULT_MODEL_BUNDLE_PATH = BASE_DIR / "models2" / "default_model_76.joblib"
ROLLING_BUNDLE_PATH = BASE_DIR / "models2" / "rolling_bundle.joblib"
...
def train_and_report(
    ...
    MODEL_OUT = BASE_DIR / "models2" / "sweep_model.joblib"
    ...
    print(f"[SAVE] wrote model bundle: {MODEL_OUT}")


    p_va = clf.predict_proba(X_va)[:, 1]
    y_hat = (p_va >= THRESHOLD).astype(int)
...
def relabel_for_combo(
    ...
    *,
    ...
    def _label_up_row(row: pd.Series, *, session: requests.Session) -> float:
        ...
        endw_raw = entry + timedelta(days=int(horizon_days))
        endw = min(endw_raw, last_closed)
        if endw < entry:
            return np.nan
        ...
        ok, missing, have_u, exp = _window_is_complete(
            w, entry, endw,
            max_missing_days=MAX_MISSING_DAYS_UP,
            min_coverage_frac=MIN_COVERAGE_FRAC,
        )
        if not ok or not _prices_are_usable(w):
            # Optional: uncomment for debugging bad periods
            # print(f"[SKIP UP] {sym} file={fday} entry={entry} end={endw} missing={len(missing)} have={have_u}/{exp}")
            return np.nan
...
def main() -> int:
    ...
    if args.sweep:
        ...
        for n_files in sweep_file_counts:
            ...
            holdout_days = 59 #max(3, int(len(stage_dates) * 0.25))  # ~25% forward holdout
            ...
            result = subprocess.run(
             [
              "python3.9",
              "score_file.py",
              "--artifact", "regime_artifact.joblib",
              "--from", "2025-11-30",
              "--to", "2026-02-06",
              "--policy", "nuanced",
              "--dir", "oversold_analysis/"
             ],
             cwd="/home/ec2-user/pythonProject1/",
             capture_output=True,
             text=True,
             check=True
            )
